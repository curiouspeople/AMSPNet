...
class AMSPNet_GCL(nn.Module):
    def __init__(self, num_classes, input_size, batch_size, dropout_rate, idx_graph, window_size, stride, out_gcn, out_gru):
        ...
        """
        super(AMSPNet_GCL, self).__init__()
...
class TemporalModule(nn.Module):
    def __init__(self, batch_size, channel, data_seg, out_gcn, out_gru, segments):
        ...
        self.gcl = GCL(data_seg, channel, out_gcn, out_gru)
    ...
    def forward(self, x):   # (b, s, c, sd)
        ...
        out_tem = self.gcl.forward(x, adj)          # (b, out_gru)
...
class GCL(nn.Module):
    def __init__(self, data_seg, channel, out_gcn, out_lstm):
        super(GCL, self).__init__()

        self.gcn = GraphConvolution(data_seg, out_gcn)
        self.out_lstm = out_lstm

        # LSTM相关的权重参数
        self.W_i = nn.Linear(out_gcn + out_lstm, out_lstm)
        self.W_f = nn.Linear(out_gcn + out_lstm, out_lstm)
        self.W_c = nn.Linear(out_gcn + out_lstm, out_lstm)
        self.W_o = nn.Linear(out_gcn + out_lstm, out_lstm)

        self.b_i = nn.Parameter(torch.zeros(out_lstm))
        self.b_f = nn.Parameter(torch.zeros(out_lstm))
        self.b_c = nn.Parameter(torch.zeros(out_lstm))
        self.b_o = nn.Parameter(torch.zeros(out_lstm))
    ...
    def forward(self, x, adj):
        ...
        # 初始化LSTM的隐藏状态和单元状态
        h_t = torch.zeros(b, c, self.out_lstm).to(DEVICE)  # (b, c, out_lstm)
        c_t = torch.zeros(b, c, self.out_lstm).to(DEVICE)  # (b, c, out_lstm)
        ...
        for t in range(s):
            ...
            gcn_out_flat = gcn_out.view(b * c, -1)

            h_t_flat = h_t.view(b * c, -1)
            c_t_flat = c_t.view(b * c, -1)

            # LSTM门控计算
            i_t = torch.sigmoid(self.W_i(torch.cat([gcn_out_flat, h_t_flat], dim=-1)) + self.b_i)  # 输入门
            f_t = torch.sigmoid(self.W_f(torch.cat([gcn_out_flat, h_t_flat], dim=-1)) + self.b_f)  # 遗忘门
            o_t = torch.sigmoid(self.W_o(torch.cat([gcn_out_flat, h_t_flat], dim=-1)) + self.b_o)  # 输出门
            c_hat_t = torch.tanh(self.W_c(torch.cat([gcn_out_flat, h_t_flat], dim=-1)) + self.b_c)  # 候选单元状态

            # 计算新的单元状态和隐藏状态
            c_t_flat = f_t * c_t_flat + i_t * c_hat_t
            h_t_flat = o_t * torch.tanh(c_t_flat)

            h_t = h_t_flat.view(b, c, -1)
            c_t = c_t_flat.view(b, c, -1)
