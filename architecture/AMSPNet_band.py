...
class AMSPNet_band(nn.Module):
    def __init__(self, num_classes, input_size, batch_size, dropout_rate, idx_graph, window_size, stride, out_gcn, out_gru):
        ...
        """
        super(AMSPNet_band, self).__init__()
    ...
    def setFFT(self, x):
        ...
        """
        # 定义五个频段
        freq_ranges = [(0.5, 4), (4, 8), (8, 13), (13, 30), (30, 50)]  # delta, theta, alpha, beta, gamma
        features = []
        ...
        input = torch.from_numpy(input).to(DEVICE)  # (b, fs, c, fd)

        for freq_range in freq_ranges:
            fmin, fmax = freq_range
            # 设计带通滤波器
            b, a = signal.butter(4, [2 * fmin / 128, 2 * fmax / 128], btype='bandpass')
            # 对数据进行滤波
            y = signal.filtfilt(b, a, input.cpu(), axis=-1)
            features.append(y)

        delta_features, theta_features, alpha_features, beta_features, gamma_features = features
        input = gamma_features

        input = torch.from_numpy(input.copy()).to(DEVICE).float()   # (b, fs, c, fd)
        input = input.permute(0, 2, 1, 3)                           # (b, c, fs, fd)
...
class GCG(nn.Module):
    def __init__(self, data_seg, channel, out_gcn, out_gru):
        ...
        # self.conv1 = nn.Conv1d(channel, channel, 64)
        # self.conv2 = nn.Conv1d(channel, channel, 32)
        # self.conv3 = nn.Conv1d(channel, channel, 14)
