...
class AMSPNet_Sim(nn.Module):
    ...
    def forward(self, x):   # (b, 1, c, d)
        ...
        out_confusion = self.attention(out_tem, out_spa)    # (b, 7*s*rc^2)/(b, out_gru)
...
class GCG(nn.Module):
    def __init__(self, data_seg, channel, out_gcn, out_gru):
        ...
        self.fc = nn.Linear(data_seg, channel)
    ...
    def forward(self, x, adj):
        ...
        for t in range(s):
            ...
            adj_t = adj + self.update_adj(curr_x)   # (b, c, c)
            adj_t = torch.sigmoid(adj_t)            # 使用 sigmoid 限制邻接矩阵的值域 (b, c, c)
    ...
    def update_adj(self, curr_x):
        """
        基于当前时刻的数据更新邻接矩阵。这里可以通过计算每对节点之间的关系来生成动态更新。
        Parameters:
        ----------
        curr_x: 当前时刻的输入数据 (b, c, sd)
        Returns:
        ----------
        adj_delta: 动态更新的邻接矩阵差异 (b, c, c)
        """
        # 示例：基于当前时刻的数据计算每对节点之间的相似性或权重
        # 可以使用一个简单的神经网络层来学习这些权重
        curr_x_flat = curr_x.view(curr_x.size(0), curr_x.size(1), -1)  # 将数据平铺成 (b, c, sd)

        # 假设使用一个全连接层来计算节点之间的相似性
        weights = self.compute_weights(curr_x_flat)  # (b, c, c)

        return weights


    def compute_weights(self, curr_x_flat):
        """
        计算当前时刻的邻接矩阵更新权重。这里可以用简单的全连接层来生成节点间的权重。
        """
        # 使用全连接层来学习节点间的相似性或关系
        b, c, _ = curr_x_flat.size()

        # 对每个节点应用全连接层，生成新的节点特征表示
        # 这里我们计算节点间的相关性
        node_features = self.fc(curr_x_flat.view(b * c, -1))  # (b * c, data_seg)

        # 然后进行重塑，重新计算节点间的权重矩阵
        node_features = node_features.view(b, c, -1)  # (b, c, data_seg)
        weight_matrix = torch.matmul(node_features, node_features.transpose(1, 2))  # (b, c, c)

        # 对结果进行处理以确保数值稳定
        adj_delta = weight_matrix  # 可以进一步通过其他处理（例如激活函数）来调整输出
