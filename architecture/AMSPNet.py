...
class AMSPNet(nn.Module):
    ...
    def forward(self, x):   # (b, 1, c, d)
        ...
        out_tem, adj_t = self.tem.forward(input)                   # (b, out_gru)    时间分支信息
        ...
        return out, tsne_out_before, tsne_out_after, adj_t
...
class TemporalModule(nn.Module):
    ...
    def forward(self, x):   # (b, s, c, sd)
        ...
        out_tem, adj_t = self.gcg.forward(x, adj)          # (b, out_gru)

        return out_tem, adj_t
...
class SpatialModule(nn.Module):
    ...
    def forward(self, x):   # (b, s, c, sd)
        ...
        out_SAB_all = torch.stack(out_SAB_all, dim=1) # (b, 7*rc, s*rc)
    ...
    def SAB(self, x):   # (7, b, rc, sd)
        ...
        outputs = torch.cat(outputs, dim=1) # (b, 7*rc, rc)
        return outputs
...
class MBFCA(nn.Module):
    ...
    def forward(self, x):   # (3, b, rc, rc)
        ...
        attention_output = attention_output.view(-1, 3, 3)  # (b, rc, rc)
